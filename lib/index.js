...
function validatePassword(pw, pp) {
  if (!pw || pw.length < pp.minLength) return { ok: false, error: 'Password must be at least ' + pp.minLength + ' characters' }
  if (pp.requireUpper && !/[A-Z]/.test(pw)) return { ok: false, error: 'Password must contain an uppercase letter' }
  if (pp.requireLower && !/[a-z]/.test(pw)) return { ok: false, error: 'Password must contain a lowercase letter' }
  if (pp.requireDigit && !/[0-9]/.test(pw)) return { ok: false, error: 'Password must contain a number' }
  if (pp.requireSpecial && !/[^A-Za-z0-9]/.test(pw)) return { ok: false, error: 'Password must contain a special character' }
  return { ok: true }
}
...
function registerAuthRoutes(ws, cfg, secret) {
  ...
  var setupHandler = async function(req, res) {
    try {
      if (hasAccounts()) return sendJson(res, 409, { ok: false, error: 'Setup already completed' })
      var ip = req.socket ? req.socket.remoteAddress : '0.0.0.0'
      var body = await parseBody(req)
      var un = String(body && body.username || '').trim()
      var pw = String(body && body.password || '')
      if (!un) return sendJson(res, 400, { ok: false, error: 'Username is required' })
      if (/[^\u4e00-\u9fa5A-Za-z0-9._@-]/.test(un)) return sendJson(res, 400, { ok: false, error: 'Username contains invalid characters' })
      var v = validatePassword(pw, cfg.passwordPolicy)
      if (!v.ok) return sendJson(res, 400, { ok: false, error: v.error })
      if (findAccount(un)) return sendJson(res, 409, { ok: false, error: 'Username already taken' })
