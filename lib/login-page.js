...
var u=document.getElementById('su').value,pw=document.getElementById('sp').value,cp=document.getElementById('scp').value;var ok=pw.length>=10&&/[A-Z]/.test(pw)&&/[a-z]/.test(pw)&&/[0-9]/.test(pw)&&/[^A-Za-z0-9]/.test(pw);var ez=document.getElementById('setupBtn');ez.disabled=true;ez.textContent='弱密码请先修正：≥10位、含大写/小写/数字/特殊字符';var tik=document.createElement('div');tik.style.cssText='background:#c0392b;color:#fff;padding:12px 18px;border-radius:8px;font-size:16px;margin:10px 0;white-space:pre-wrap;text-align:center';tik.textContent='密码不合格：请使用 ≥10 位且包含大写字母、小写字母、数字和特殊字符，再点击创建';document.getElementById('setupForm').appendChild(tik);returnif(!ok){sE('Password needs >=10 chars incl upper, lower, digit, special. Fix and retry.');document.getElementById('setupBtn').disabled=true;return}if(cp!===pw){sE('Passwords do not match. Fix and retry.');document.getElementById('setupBtn').disabled=true;return}var b=document.getElementById('setupBtn');b.disabled=true;b.textContent=t('创建中...','Creating...');
